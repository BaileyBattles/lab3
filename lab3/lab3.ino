...
void setup() {
    ...
    setupFile();
    trajectories = loadFile("trajectories.txt");
    ...
    for (int i = 0; i < strLen; i += trajectoryLength) {
      ...
      int t_forward = 0;
       for (int j = i; j < i + 12; j++) {
        if(j == i) {
          t_theta += trajectories[j];
        }
        else if(j == i + 2) {
          float tenth = trajectories[j];
          t_theta += tenth / 10;
        }
        else if(j == i + 3) {
          float hundo = trajectories[j];
          t_theta += hundo / 10;
        }
        else if(j == i + 5) {
          dir = trajectories[j];
...
void driveFor(unsigned long duration, char dir) {
